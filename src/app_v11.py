"""
Fischer Energy Partners - Data Processing Application V11
Fully Automatic Workflow with Progress Tracking
...
- Streamlined 4-step process (down from 6 steps)
- Removed AI Debug Panel for cleaner interface
- All V10 features: Tab-based UI, smart data types, enhanced flagging, multi-tab Excel
"""
...
# Page configuration
st.set_page_config(
    page_title="Fischer Data Processing - V11",
    page_icon="📊",
    layout="wide"
)
    ...
    if logo_path.exists():
        ...
        st.markdown(
            ...
            <div style="display: flex; align-items: center; padding: 10px 0; margin-bottom: 20px;">
                ...
                <h1 style="color: #24b3aa; margin: 0; font-size: 2.5rem;">Fischer Data Processor V11</h1>
            </div>
            """,
            unsafe_allow_html=True
        )
    else:
        st.title("🔧 Fischer Data Processor V11")
    ...
    try:
        ...
        for idx, (file_name, config) in enumerate(file_configs.items()):
            ...
            # CSV or single-tab Excel file
                ...
                # Get value column with smart conversion
                if inner_config['value_column'] < len(df_full.columns):
                    value_col_name = df_full.columns[inner_config['value_column']]
                    df_clean[inner_config['sensor_name']] = smart_convert_column(
                        df_full[value_col_name], threshold=0.8
                    )
...
def main():
    ...
    st.markdown("**Automatic Workflow** • **Tab-Based UI** • **Smart Data Types** • **Enhanced Flagging** • **4-Step Process**")
    st.markdown("---")
            ...
            if not st.session_state.processing_complete:
                ...
                **All files will be saved to:** `{st.session_state.archive_path}`
                ...
                with col1:
                    ...
                    st.metric("Total Intervals", f"{total_rows:,}")
                with col2:
